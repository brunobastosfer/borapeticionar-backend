...
let AuthService = class AuthService {
    ...
    async login(user, userAgent, ipAddress) {
        ...
        if (activeSession) {
            throw new common_1.ConflictException('Você já está logado em outra máquina. Faça logout antes de acessar de outro dispositivo.');
        }
        const payload = {
            sub: user.id,
    ...
    async refreshTokens(userId, oldRefreshToken) {
        ...
        if (!storedToken || storedToken.userId !== userId) {
            throw new common_1.ForbiddenException('Refresh token inválido');
        }
